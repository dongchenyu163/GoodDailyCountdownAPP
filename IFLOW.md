...
### iOS 应用
- 构建和运行: 
  - 不在dongchenyu165的机器上开发，忽略该平台的编译运行。
  - 仅在feibi2015的机器上开发。
...
- **开发、修复的流程**: 
  ...
  4. **编译验证**：这个时候的验证只需要编译Desktop平台（即：jvm ）的，其他平台的编译不需要验证。
  ...
  6. **循环或结束初步修改**：
     - 如果还有任务未执行，则返回第2步继续
     - 如果所有任务完成，则进行步骤7
  7. **（询问用户）相关平台编译验证**：询问用户是否需要相关平台（目前不考虑`jsMain`, `wasmJsMain`）编译验证
     - 如果用户不需要编译验证，则结束流程。
     - 目前来讲只考虑`androidMain`, `iosMain`平台编译验证，忽略【Web应用】即：`jsMain`, `wasmJsMain`平台编译验证
     - 首先确定上述任务的编辑受影响的平台。仅针对需修改的平台进行编译错误的验证
       - 确定方法是：查看更改的文件，是否处于各个平台特定的文件夹中（例如：`androidMain`, `iosMain`）。
       - 平台特定的文件夹的搜索范围仅限于`composeApp/src`下
